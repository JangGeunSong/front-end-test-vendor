...
- `tools/ai-generator/classify_interaction_candidates.py`: 기존 artifact의 action 후보를 safe/unsafe/unknown으로 분류하고 canonical dedup identity 기반 `candidateKey`를 부여하며 producer의 actual `observedUrl`을 required evidence로 보존한다.
...
- `tools/ai-generator/validate_interaction_approvals.py`: human-authored approval artifact의 schema version, target scope, decision, evidence snapshot, reviewer metadata, duplicate와 unknown field를 strict하게 검증한다.
- `tools/ai-generator/reconcile_interaction_approvals.py`: validated approval과 Analysis Review Report의 current candidate를 exact `candidateKey`/evidence로 대조해 deterministic reference status, eligibility, unreviewed candidate를 생성한다.
...
Future tab restore implementation responsibility:
...
위 future responsibility는 documentation contract이며 current source implementation은 여전히 report/approval/reconciliation/plan `2.0`과 tab `reloadPage` renderer다.
...
- future: explicit tablist group selector와 selected peer evidence 수집

`scout.js`는 테스트 코드를 생성하지 않는다. 데이터 변경 액션도 수행하지 않는다. Current `collectTabs()`는 individual tab selector/state만 수집하며 future group/peer evidence는 아직 구현되지 않았다.
