...
HMV-001 engine invocation adapter, HMV-002 run-scoped workspace, HMV-003 internal artifact manifest와 HMV-004 normalized terminal result가 완료됐다. 다음 권장 milestone은 기존 Local recovery message와 private diagnostics를 유지하면서 stable error category boundary를 추가하는 HMV-005다. Hosted API, Web UI, persistence, dispatcher와 public execution은 아직 포함하지 않는다.
