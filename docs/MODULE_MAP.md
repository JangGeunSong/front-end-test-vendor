...
- `tools/ai-generator/interaction_url.py`: approval/reconciliation/plan 계층이 공유하는 absolute credential-free HTTP(S)와 same-origin validation을 제공하며 URL을 normalize하지 않는다.
- `tools/ai-generator/classify_interaction_candidates.py`: 기존 artifact의 action 후보를 safe/unsafe/unknown으로 분류하고 canonical dedup identity 기반 `candidateKey`를 부여하며 producer의 actual `observedUrl`, bounded `tabRestore` 또는 unavailable reason을 보존한다.
- `tools/ai-generator/build_analysis_review_report.py`: navigation, Page Identity, interaction evidence를 schema `2.1` deterministic JSON review artifact로 구성하고 tab restore readiness count를 제공한다.
- `tools/ai-generator/render_analysis_review_report.py`: report JSON과 bounded target/restore pair를 사람이 읽을 수 있는 deterministic Markdown으로 렌더링한다.
- `tools/ai-generator/validate_interaction_approvals.py`: human-authored approval schema `3.0`의 target scope, decision, bounded tab pair evidence, reviewer metadata, duplicate와 unknown field를 strict하게 검증한다.
- `tools/ai-generator/reconcile_interaction_approvals.py`: validated approval과 Report `2.1`의 current target/restore evidence를 exact key/path로 대조해 schema `3.0` deterministic reference status, eligibility, unreviewed candidate를 생성한다.
- `tools/ai-generator/interaction_plan_contract.py`: interaction plan schema/template/ID constant와 reconciliation/report exact input binding을 builder/validator에 공유한다.
- `tools/ai-generator/build_interaction_plan.py`: reconciliation eligible candidate만 report evidence와 exact `candidateKey`로 join하고 `observedUrl`을 per-test `startUrl`로 exact copy해 supported bounded plan case를 생성한다.
- `tools/ai-generator/validate_interaction_plan.py`: plan schema, eligible membership, exact startUrl/evidence copy, template compatibility, deterministic ID/order와 bounded state/reset을 strict하게 검증한다.
- `tools/ai-generator/render_interaction_plan.py`: validated schema `3.0` plan만 읽고 tab의 exact target/restore selector paired transition과 expandedToggle의 existing same-target reset을 deterministic CommonJS Playwright spec으로 렌더링한다. Report, approval, reconciliation을 다시 읽거나 browser를 실행하지 않는다.
- `tools/ai-generator/write_interaction_approvals.py`: 선택된 current report candidate만 exact evidence snapshot으로 복사해 Approval `3.0`을 atomic write하고 기존 validator를 통과시키는 deterministic local helper다.

## Local MVP Modules

- `tools/mvp/controller.js`: 기존 Python/Playwright command를 run 단위로 serialize하고 stage state, normalized analysis/result와 artifact lifecycle을 조합한다.
- `tools/mvp/server.js`: analyze/status/analysis/approve/execute/result/report 책임이 분리된 localhost HTTP API와 static UI를 제공한다.
- `tools/mvp/public/`: URL 입력, evidence review, explicit approval, 진행 상태, result와 HTML report link를 제공하는 dependency-free UI다.
- `tools/mvp/playwright.config.js`: product run에서 기존 timeout을 늘리지 않고 headless, workers 1, retries 0, trace on 조건을 고정한다.

Tab restore responsibility:

- `scout.js` pageProfile producer는 closest explicit `role=tablist` ancestor selector, group membership과 exactly-one selected peer evidence를 수집한다. Explicit relation이 없으면 만들지 않는다.
- classifier/report builder는 safety classification을 바꾸지 않고 optional bounded `tabRestore` evidence를 preserve/표시한다.
- approval validator는 approved tab의 human-reviewed target/restore pair shape를 검증한다.
- reconciler는 current target과 restore peer/group evidence를 exact compare하고 existing `missingCandidate`/`evidenceChanged` status로 stale을 표현한다.
...
Human approval artifact의 schema와 classifier/report/future plan 사이 책임 경계는 `docs/INTERACTION_APPROVAL_CONTRACT.md`가 소유한다. 기본 manual local state 경로는 `tools/ai-generator/review/interaction_approvals.json`이다. MVP writer는 run별 generated directory에 current exact snapshot을 저장하며 general-purpose durable editor/history는 제공하지 않는다. Reconciliation output도 run별 generated artifact로 분리한다.
...
- root rendered DOM candidate의 final `window.location.href`와 pageProfile click 후 `page.url()` provenance 수집
...
- `STRUCTURED_INTERACTION_PLAN.md`: eligible candidate reference, bounded state, reset/restore와 builder/validator/future renderer 경계
