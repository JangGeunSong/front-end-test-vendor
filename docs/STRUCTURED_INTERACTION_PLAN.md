...
구현된 schema `3.0` builder, validator, renderer는 `interaction.tabSelection`의 approved/current target+restore pair를 exact하게 소비하고 `restorePreviousSelection` fixed spec을 생성한다. Schema `2.0`의 `reloadPage` tab restore는 제거됐다. Static syntax와 Playwright discovery는 검증됐지만 browser runtime PASS는 아직 완료되지 않았다.
