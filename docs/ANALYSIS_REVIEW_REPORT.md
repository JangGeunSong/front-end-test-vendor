...
- future report contract `2.1`은 deterministic evidence가 있는 unselected tab에 optional bounded `tabRestore` evidence를 표시한다. Producer는 아직 구현되지 않았다.
...
Future report version `2.1`은 safe unselected tab에서 다음 deterministic evidence가 모두 확인될 때만 `tabRestore`를 포함한다.
...
같은 parent, sibling, class name, text similarity, common selector prefix 또는 같은 페이지의 first selected tab으로 relation을 채우지 않는다. Current report `2.0`은 individual tab evidence만 소유하며 tab group relation을 표현할 수 없다.
...
- tab group/previous selected peer producer와 report `2.1` implementation
