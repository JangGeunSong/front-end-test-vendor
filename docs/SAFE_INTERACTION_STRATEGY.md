...
- 첫 schema `2.0` tab browser run은 reload 후 selected state가 유지돼 restore에 실패했고 `reloadPage`를 제거하는 근거가 됐다.
- Fresh explicit-tablist evidence, Approval/Reconciliation `3.0`, Plan `3.0`과 exact `restorePreviousSelection` renderer chain은 target/restore paired state를 retry 없이 2회 browser PASS했다.
...
Current implementation schema `3.0`은 두 template만 유지하며 각 test는 exact candidate `observedUrl`을 `startUrl`로 가진다. Tab은 exact previous selected peer를 복원하고 expandedToggle은 same target toggle을 유지한다.
...
- Analysis Review Report `2.1` optional `tabRestore`와 unavailable reason
- approval/reconciliation schema `3.0`

Static implementation 완료:

- Structured Interaction Plan schema `3.0`
- deterministic `restorePreviousSelection` renderer와 Playwright discovery

Runtime boundary:

- `interaction.tabSelection` previous-selection restore: fresh public target에서 2회 PASS
- `interaction.expandedToggle`과 cross-site interaction runtime: not implemented
