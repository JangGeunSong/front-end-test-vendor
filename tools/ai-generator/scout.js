...
async function collectPageProfile(page) {
  return page.evaluate(() => {
    ...
    function isInsideCommonLayout(el) {
      return !!el.closest(
        'header, footer, nav, aside, .header, .footer, .gnb, .lnb, nav, .menuContent, .depth1, .depth2, .depth3'
      );
    }
...
async function scoutSite(url) {
  ...
  const scoutResult = await page.evaluate(() => {
    ...
    const depth1Items = Array.from(document.querySelectorAll('nav > ul > li'));
    const menuContentPanels = Array.from(document.querySelectorAll('nav .menuContent'));

    function inferDepth1Index(el) {
      const depth1Item = el.closest('nav > ul > li');
      if (depth1Item) {
        const index = depth1Items.indexOf(depth1Item);
        return index >= 0 ? index : null;
      }

      const menuContentPanel = el.closest('nav .menuContent');
      if (menuContentPanel) {
    ...
    const elements = Array.from(document.querySelectorAll(selectors))
      .map((el, index) => {
        ...
        const isGnbCandidate = !!el.closest(
          'nav, .menuContent, .depth1, .depth2, .depth3'
        );
