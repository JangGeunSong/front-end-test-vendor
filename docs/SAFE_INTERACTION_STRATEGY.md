...
- eligible candidate를 future renderer에 전달할 Structured Interaction Plan schema `1.0` documentation contract가 정의되어 있다.
- 실제 browser interaction과 Level 3 test generation은 아직 구현하지 않는다.
...
Schema `1.0` documentation contract가 지원하는 template:
...
- structured interaction plan builder/validator
