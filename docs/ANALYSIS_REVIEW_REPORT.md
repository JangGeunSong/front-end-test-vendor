...
Report는 future interaction plan의 eligibility를 직접 결정하지 않는다. Approval reconciliation이 eligible candidate의 source of truth이며, report의 exact candidate ARIA state evidence는 future builder/validator가 bounded initial state를 확인하는 보조 입력으로만 사용한다. 상세 경계는 [STRUCTURED_INTERACTION_PLAN.md](STRUCTURED_INTERACTION_PLAN.md)를 따른다.
