...
구현된 schema `2.0` builder, validator, renderer는 `interaction.tabSelection`에 `reloadPage`를 사용한다. 첫 browser runtime에서 target transition은 통과했지만 reload 후 selected state가 유지되어 restore가 실패했다. 이 문서는 그 결과를 반영한 **future Plan schema `3.0` contract**를 정의한다. Tab producer, Report `2.1`, Approval/Reconciliation `3.0`은 구현됐고 Plan builder, validator, renderer와 runtime PASS는 아직 구현되지 않았다.
...
```json
{
  "tabRestore": {
    ...
    "target": {
      ...
      "selector": "#tab-product-b",
      "observedUrl": "https://example.test/sample/tabs",
      "pageContext": "Sample tabs",
      "role": "tab",
      "tagName": "button",
      "text": "Product B",
      "ariaAttributes": {
...
Migration framework는 만들지 않는다. Approval/Reconciliation validator는 old `2.0`을 unsupported version으로 명시적으로 거부한다. Plan source constants와 fixtures는 여전히 `2.0`이며 후속 implementation에서 변경해야 한다.
  ...
  "tests": [
    {
      ...
      "pageContext": "Sample tabs",
      "startUrl": "https://example.test/sample/tabs",
      "target": {
        "selector": "#tab-product-a",
        "interactionKind": "tab",
        "tabGroupSelector": "main [role='tablist']"
