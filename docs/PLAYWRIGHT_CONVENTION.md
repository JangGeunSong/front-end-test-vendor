...
Current Structured Interaction Plan schema `3.0` renderer는 test별 exact `startUrl`과 selector를 사용한다. Tab renderer는 validated interaction target selector와 restore target selector를 각각 `page.locator()`에 exact 전달하고 paired selected state를 검증한다. ExpandedToggle의 same-target toggle contract는 유지한다.
