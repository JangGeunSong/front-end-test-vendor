...
- `tools/ai-generator/build_interaction_plan.py`: reconciliation eligible candidate만 report evidence와 exact `candidateKey`로 join해 supported bounded plan case를 생성하고 unsupported reason을 CLI summary로 분리한다.
- `tools/ai-generator/validate_interaction_plan.py`: plan schema, eligible membership, exact evidence copy, template compatibility, deterministic ID/order와 bounded state/reset을 strict하게 검증한다.
