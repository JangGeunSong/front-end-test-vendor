...
def build_menu_test_prompt(generation_input):
    ...
    3-3. child JSON에 id, ngClick, cssPath가 있으면 options에 반드시 포함한다. cssPath가 있으면 절대 생략하지 않는다.
         예: clickVisibleSubMenuByText(page, 'Support A', 'Guide A', {{ cssPath: 'li#supportDevMain > a' }})
    ...
    3-2. children 배열의 각 child는 text, href 또는 ngClick, id, cssPath를 menu_map 값 그대로 literal field로 포함한다.
         허용 예: {{ text: 'Product A', id: '4_1', ngClick: "selectMenu('product-a')", cssPath: "a#\\\\34 _1" }}
    ...
    3-5. clickVisibleSubMenuByText options에서는 계산식이 아니라 child.cssPath를 사용한다.
         허용 예: clickVisibleSubMenuByText(page, 'Products', child.text, {{ id: child.id, ngClick: child.ngClick, cssPath: child.cssPath }})
    ...
    3-4. 여러 메뉴에 공통으로 쓸 content selector를 임의 생성하지 않는다. Resources > Resource A/Resource B/FAQ처럼 안정적인 content cssPath를 하나 고르기 어렵다면 assertion과 highlight를 만들지 말고 TODO 주석만 남긴다.
    ...
    7. Guide A/Guide B처럼 heading이 부모 메뉴명만 있는 경우에는 수집된 mainContainers[1] 또는 content cssPath를 그대로 사용해 visible assertion과 highlightPageIdentity를 생성한다.
    ...
    5. synthetic product Products, Product A depth3 메뉴처럼 URL/hash가 동일한 ngClick tab 메뉴에서도 Page Identity highlight를 반드시 생성한다.
    ...
    8. label에는 menuPath 전체를 포함한다. 예: 'Product A > Product A: content area'
    9. buttons, table, 공지 제목, FAQ 질문, 제품명/모델명, 상세보기 버튼은 assertion 또는 Page Identity highlight 대상으로 사용하지 않는다.
    10. 예:
       const identityHeading = page.getByRole('heading', {{ name: 'Product A' }});
       await expect(identityHeading).toBeVisible();
       await highlightPageIdentity(page, identityHeading, 'Product A > Product A: Product A');
    11. mainContainer 예:
       const identityArea = page.locator('main').first();
       await expect(identityArea).toBeVisible();
       await highlightPageIdentity(page, identityArea, 'Product A > Product A: content area');
...
if __name__ == "__main__":
    target_url = "https://public-navigation.example.com" # 실제 테스트 대상 URL
